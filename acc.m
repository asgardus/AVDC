  ...
  figure(2);
    ...
    for j=i:length(veh_x.time)      %loop for finding shortest braking distance #gives k_b
        ...
        if veh_x.Data(j) <= 0.000
            b_t(d)=veh_x.time(j)-(3+acc_t(d)); %add acc_t when removing for loop %the time taken from application of brake to vehicle halt
            b_d(d)=veh_dist.data(j)-(75+acc_d(d)); %add acc_d when removing for loop%the dist taken from application of brake to vehicle halt
